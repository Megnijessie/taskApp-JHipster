...
/**
 * REST controller for managing {@link com.mwc.task.domain.Task}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class TaskResource {
    ...
    public ResponseEntity<Task> createTask(@Valid @RequestBody Task task) throws URISyntaxException {
        ...
        if (Objects.isNull(task.getCreer())) {
            throw new BadRequestAlertException("Invalid association value provided", ENTITY_NAME, "null");
        }
        Long userId = task.getCreer().getId();
        userRepository.findById(userId).ifPresent(task::user);
        ...
        Optional<Task> result = taskRepository
            ...
            .map(existingTask -> {
                ...
                if (task.getDateStart() != null) {
                    existingTask.setDateStart(task.getDateStart());
                }
                if (task.getDateEnd() != null) {
                    existingTask.setDateEnd(task.getDateEnd());
                }
